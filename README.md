...
**File/folder count**: no limit. The frontend (`PlusFiles.jsx`) sets no `maxFiles` on the dropzone and uploads each file as its own request (for per-file progress/retry), so `PLUS_MAX_FILES_PER_UPLOAD` above never comes into play — a folder with thousands of files queues and uploads them all, three at a time (`CONCURRENCY` in `PlusFiles.jsx`). The ceiling is `filesUploadLimiter`'s rate limit (`middleware/rateLimiter.js`, `FILES_UPLOAD_LIMIT_MAX_REQUESTS`, default 100,000 requests/hour, per IP) — its own bucket, **not** shared with Document uploads. It used to reuse `uploadLimiter` (the Document upload limiter, 20,000/hour); since the vault sends one request per file with no count cap, a single ~13,000-file folder already burned most of that shared budget, and any other upload activity on the same IP in that hour (retries, a second batch, unrelated Document uploads) could tip it over — surfacing as a handful of files scattered through a big batch failing, with no relationship to their size. Splitting the buckets fixes that.
